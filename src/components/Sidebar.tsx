...
//       {/* PANEL */}
//       {isNotifOpen && (
//         <div className="fixed left-16 top-0 h-screen w-80 xl:w-100 bg-black border-r border-gray-800 shadow-2xl z-65 animate-in slide-in-from-left duration-300">
...
//MOBIL BOYUTTA NAVBAR ASAGIYA GELIYOR 
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-screen border-r border-gray-800 bg-black p-3 hidden md:flex flex-col transition-all duration-300 z-70 ${isNotifOpen ? "w-16" : "w-16 xl:w-64"}`}>
        <div className="mb-10 px-3 pt-6">
          <Link href="/">
            <h1 className={`text-xl font-bold italic text-yellow-500 ${isNotifOpen ? "hidden" : "hidden xl:block"}`}>MovieSocial</h1>
            <div className={`w-8 h-8 bg-yellow-500 rounded-lg flex items-center justify-center text-black font-black ${isNotifOpen ? "block" : "xl:hidden"}`}>M</div>
      ...
      {/* MOBILE BOTTOM NAV (Telefonlarda aşağıda - Instagram stili) */}
      <nav className="fixed bottom-0 left-0 right-0 h-16 bg-black border-t border-gray-800 flex md:hidden items-center justify-around px-2 z-70 pb-safe">
        <Link href="/" className={pathname === '/' ? "text-yellow-500" : "text-gray-400"}>
          <Home size={26} />
        </Link>
        <button onClick={() => setIsSearchOpen(true)} className="text-gray-400">
          <Search size={26} />
        </button>
        <Link href="/explore" className={pathname === '/explore' ? "text-yellow-500" : "text-gray-400"}>
          <Compass size={26} />
        </Link>
        {isAuthenticated && (
          <button onClick={() => setIsNotifOpen(!isNotifOpen)} className={`relative ${isNotifOpen ? "text-yellow-500" : "text-gray-400"}`}>
            <Bell size={26} />
            {unreadCount > 0 && (
      ...
      <Link href={`/profile/${user?.username}`} className={`cursor-pointer ${pathname.includes('/profile') ? "text-yellow-500" : "text-gray-400"}`}>
        <User size={26} />
      </Link>
      {/* Mobilde Logout'u küçük bir ikon olarak profilin yanına ekleyebilirsin veya Profil sayfasının içine koyabilirsin */}
      <button onClick={() => { logout(); router.push("/login"); }} className="text-red-500 cursor-pointer p-1">
        <LogOut size={20} />
      </button>
    </div>
        ) : (
      ...
      {/* NOTIFICATION PANEL (Mobile özel: Tam ekran veya geniş modal gibi açılır) */}
      {isNotifOpen && (
      ...
      {isSearchOpen && <SearchModal onClose={() => setIsSearchOpen(false)} />}
      {isNotifOpen && <div className="fixed inset-0 bg-black/50 md:bg-transparent z-64" onClick={() => setIsNotifOpen(false)} />}
    </>
  );
}
...
export default function SearchModal({ onClose }: { onClose: () => void }) {
  ...
  const renderItem = (item: SearchResultDto) => {
    ...
    return (
      ...
      >
        <div
          className={`relative shrink-0 bg-gray-900 overflow-hidden shadow-lg flex items-center justify-center ${isMovie ? "w-10 h-14 rounded-md" : "w-12 h-12 rounded-full border border-gray-800"}`}
        >
          {displayImage ? (
            <img
              src={displayImage}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 truncate">
            {item.subTitle || (isMovie ? "Movie" : "User")}
          </p>
          {item.description && (
        ...
        <ChevronRight
          size={14}
          className="text-gray-700 group-hover:text-yellow-500 transform group-hover:translate-x-1 transition-all"
        />
      </Link>
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-100 flex animate-in fade-in duration-300">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative ml-16 xl:ml-64 w-full max-w-md h-full bg-[#030712] border-r border-gray-900 shadow-2xl flex flex-col animate-in slide-in-from-left duration-500 text-white">
        <div className="p-6 border-b border-gray-900">
          ...
          <div className="relative">
            {isLoading ? (
              <Loader2
                className="absolute left-3 top-1/2 -translate-y-1/2 text-yellow-500 animate-spin"
                size={18}
              />
            ) : (
              <SearchIcon
            ...
            <input
              ...
              className="w-full bg-gray-900 border border-gray-900 rounded-xl py-3 pl-10 pr-4 outline-none focus:ring-2 focus:ring-yellow-500/50 text-sm transition-all text-white"
          ...
          {results && (
            <div className="space-y-6">
              {results.movies.length > 0 && (
                <section>
                  <h3 className="px-4 mb-2 text-[10px] font-black text-gray-500 uppercase tracking-widest">
                    {t("search.movies")}
                  </h3>

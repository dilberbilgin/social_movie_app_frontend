...
export default function MovieCard({ movie: initialMovie }: { movie: Movie }) {
  ...
  useEffect(() => {
    setMovie(initialMovie);
    setImgError(false); // Yeni film geldiğinde hata durumunu sıfırla
  }, [initialMovie]);
  ...
  return (
    <Link href={`/movies/${movie.id}?tmdbId=${movie.tmdbId}`} 
      className="group block w-32 md:w-40 shrink-0">
      <div className="aspect-2/3 relative rounded-2xl overflow-hidden border border-gray-800 group-hover:border-yellow-500/50 transition-all shadow-xl bg-gray-900 flex items-center justify-center">
        
        {/* Poster Resmi veya Fallback */}
        {!imgError && safePoster ? (
          <img 
            src={safePoster} 
            alt={movie.title}
          ...
          <div className="flex flex-col items-center gap-2 opacity-40">
            ...
            <span className="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">No Poster</span>
          </div>
        )}

        {/* ALT BİLGİ ŞERİDİ */}
        <div className="absolute bottom-0 inset-x-0 bg-linear-to-t from-black via-black/80 to-transparent px-3 py-3 flex justify-between items-center z-20">

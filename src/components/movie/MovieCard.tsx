...
export default function MovieCard({ movie: initialMovie }: { movie: Movie }) {
  ...
  const getSafePosterUrl = () => {
    ...
    if (!url || url.includes("via.placeholder.com") || url === "null" || url === "") return null;
  ...
  return (
    <>
      ...
      >
        {/* KRİTİK DÜZELTME: 
           - 'overflow-hidden' kaldırıldı (Menünün açılabilmesi için).
           - Resme 'rounded-2xl' eklendi.
           - border-2 her zaman var, sadece rengi değişiyor (titremeyi engeller).
        */}
        <div className="aspect-2/3 relative rounded-2xl border-2 border-gray-800 group-hover:border-yellow-500 transition-all duration-300 shadow-xl bg-gray-900 flex items-center justify-center">
          ...
          <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black via-black/90 to-transparent px-3 py-3 flex justify-between items-center z-20 rounded-b-[14px]">
            ...
            <div className="flex items-center gap-2"> {/* Like ve Menü yanyana daha düzgün durur */}
              <button 
                onClick={handleLike}
                disabled={isLiking}
                className={`

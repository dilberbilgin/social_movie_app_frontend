...
//       // Backend'den gelen yeni UUID ile üst componenti bilgilendiriyoruz
...
//             <button
//               onClick={onSaveClick}
//               className="flex items-center gap-2 px-6 py-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-full hover:bg-white/20 transition-all text-white font-semibold"
//             >
...
//           {/* Dil Seçici: Sağ tarafa yaslı ve yukarı açılır */}
//           <LanguageSelector
...
import { Movie } from "@/types";
import { Heart, Skull, Bookmark } from "lucide-react"; // Bookmark ikonunu da ekleyelim
import { movieService } from "@/services/movieService";
import LanguageSelector from "../ui/LanguageSelector";
import { useTranslation } from "@/context/LanguageContext"; // t fonksiyonu için import

interface MovieHeroProps {
  movie: Movie;
  onMovieUpdated: (newId?: string) => void;
  onSaveClick: () => void; // 1. Hata: Prop tipini buraya ekledik
}

export default function MovieHero({ movie, onMovieUpdated, onSaveClick }: MovieHeroProps) {
  const { t } = useTranslation(); // 2. Hata: t fonksiyonunu tanımladık
  ...
  return (
    <div className="relative w-full h-80 bg-gray-950 overflow-hidden">
      {/* Arka plan atmosferi - Kodun geri kalanı aynı kalacak... */}
      <div className="absolute inset-0 z-0">
        {movie.posterUrl && (
          <img
            src={movie.posterUrl}

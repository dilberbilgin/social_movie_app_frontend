...
export default function MovieDetailContent({ id }: { id: string }) {
  ...
  const { user } = useAuth();
  const { movie, comments, loading, error, setMovie, fetchAllData } = useMovieDetail(id);
  ...
  if (loading) return <div className="p-20 text-center text-yellow-500">Loading...</div>;
      ...
      <div className="max-w-6xl mx-auto px-4 mt-12 grid grid-cols-1 lg:grid-cols-3 gap-10 pb-10">
        <div className="lg:col-span-2 space-y-10">
          <section className="bg-gray-800/20 p-6 rounded-2xl border border-gray-800/50">
            <h2 className="text-yellow-500 font-bold mb-4 uppercase text-sm">{t("movie.description")}</h2>
            <p className="text-gray-300 text-lg leading-relaxed">{movie.description}</p>
          </section>
          {comments && (
            <CommentSection movieId={movie.id} initialData={comments} totalCount={movie.commentCount || 0} />
          )}
        </div>
        <aside className="space-y-6">
          <MovieStats movie={movie} />
          {user ? (
            <RatingAction movieId={movie.id} initialScore={movie.userScore} onRatingSuccess={updateLocalMovieStats} />
          ) : (

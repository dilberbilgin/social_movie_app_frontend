...
export const movieService = {
  ...
  searchMovies: async (
    filters?: { genreId?: string; title?: string },
    page: number = 0,
    size: number = 20,
  ): Promise<RestResponse<PageResponse<Movie>>> => {
  ...
  importTmdbMovie: async (tmdbId: number): Promise<RestResponse<Movie>> => {
    const response = await api.post<RestResponse<Movie>>(
      `/tmdb/import/${tmdbId}`,
    );
    return response.data;
  },
  ...
  getTrendingReviews: async (
    limit: number = 4,
  ): Promise<RestResponse<TrendingReview[]>> => {
    const response = await api.get<RestResponse<TrendingReview[]>>(
      "/v1/weekly-winners/trending",
      {
        params: { limit },
      },
    );
